...
/*
const objLat = (obj) => {
return output = `my name is ${firstName}   ${lastName}I am ${age} YO, and I love ${hobby}.`
...
const cvFormatter = (arr) => {
    ...
    let arr2 = [];
    arr.forEach((arrayItem, i) => {
        if (arrayItem.yearsOfExperience > 1) {
            if (arrayItem.firstName === null || arrayItem.lastName === null) {
                if (!arrayItem.firstName) {
                    arrayItem["fullName"] = arrayItem.lastName;
                }
                else {
                    arrayItem["fullName"] = arrayItem.firstName;
                }
            ...
            delete arrayItem.lastName;
            delete arrayItem.firstName;
            delete arrayItem.yearsOfExperience;
        } else {
            arr2.push(i)
        }
    })
...
const applicationsStatics = (arr) => {
    ...
    for(var i =0 ; i < data.grades.length;i++)
    {
        for (var j = 0 ; j <data.grades[i].classes.length;j++)
        {
            ...
            {
                sumOfMarks+=data.grades[i].classes[j].classScores[x];
                numberOfMarks+=1;

            }
            data.grades[i].classes[j].avg=Math.floor(sumOfMarks/numberOfMarks);
