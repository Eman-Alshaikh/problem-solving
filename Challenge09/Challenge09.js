...
// 3) ---------------------
// 
//  An owner of a factory want to give bounce 100$ for the employee of production department who worked
// *More than* 8 hours
...
const employeesBonus = (arr) => {
  ...
  return arr.map(emp => {
    ...
    return{
      ...
      salary: parseInt(emp.salary.substring(0, emp.salary.length)) +bounus + "$"
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
  ...
  for (let i = 0; i < mouseArray.length; i++) {
    for (let j = 0; j < keyBoardArray.length; j++) {
      let Totalprice = mouseArray[i] + keyBoardArray[j]
